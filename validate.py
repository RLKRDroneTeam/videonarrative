...
if __name__ == '__main__':
    ...
    if cfg.test.write_preds:
        ...
        if cfg.dataset.question_type in ['action', 'transition','none']: 
            ...
            np_ids = np.array(org_q_ids)
            print("shape of org_q_ids: " + str(np_ids.shape))
            print("dtype of org_q_ids: " + str(np_ids.dtype))
            
            for idx in range(len(org_q_ids)+1):
                q_ids = list(map(lambda x:x.cpu(), q_ids))
                #print(f'idx: {idx}, has idx in -- org_v_ids: {idx in org_v_ids}, questions: {idx in questions}, ans_candidates: {idx in ans_candidates}')
                if idx < len(org_q_ids) and idx < len(org_v_ids):
